...
%     x_nad = max(P.decs);
    ...
    while Global.NotTermination(P)
        ...
        TP = INDIVIDUAL;
        for i = 1:n
            [Offspring,a,GNI,Q] = GQ(P(i),P,Q,G,Dis,K,epsilon,F,d,isg,loi);
            if i == 1
                Q = Q(2:end,:);
            end
            r = GR(Offspring,Global.PF);
            Q = UpdateQ(P(i),a,P,G,r,K,Dis,Q,gamma,alpha,GNI);
            TP = [TP,Offspring];
        end
        TP = TP(2:end);
